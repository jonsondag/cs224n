...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        for word_idx in range(len(sentence)):
            desired_begin_idx = word_idx - window_size
            actual_begin_idx = max(0, desired_begin_idx)
            desired_end_idx = word_idx + window_size + 1
            actual_end_idx = min(len(sentence), desired_end_idx)
            start_tokens = start * (actual_begin_idx - desired_begin_idx)
            sentence_tokens = []
            for i in range(actual_begin_idx, actual_end_idx):
                sentence_tokens += sentence[i]
            end_tokens = end * (desired_end_idx - actual_end_idx)
            windowed_data.append((start_tokens + sentence_tokens + end_tokens, labels[word_idx]))
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        embedding_tensor = tf.get_variable("embedding", initializer=self.pretrained_embeddings)
        input_embeddings = tf.nn.embedding_lookup(embedding_tensor, self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable("W", shape=[self.config.n_window_features * self.config.embed_size, hidden], initializer=tf.contrib.layers.xavier_initializer())
        b1 = tf.get_variable("b1", shape=(hidden, ), initializer=tf.contrib.layers.xavier_initializer())
        U = tf.get_variable("U", shape=(hidden, classes), initializer=tf.contrib.layers.xavier_initializer())
        b2 = tf.get_variable("b2", shape=(classes, ), initializer=tf.contrib.layers.xavier_initializer())
    ...
    def add_loss_op(self, pred):
        ...
        loss_by_example = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=pred)
        loss = tf.reduce_mean(loss_by_example)
        ### END YOUR CODE
        return loss
